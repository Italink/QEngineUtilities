...
void QPixelFilterRenderPass::resizeAndLink(const QSize& size, const TextureLinker& linker) {
	mRT.colorAttachment.reset(mRhi->newTexture(QRhiTexture::RGBA32F, linker.getInputTexture(InSlot::Src)->pixelSize() , 1, QRhiTexture::RenderTarget | QRhiTexture::UsedAsTransferSource));
