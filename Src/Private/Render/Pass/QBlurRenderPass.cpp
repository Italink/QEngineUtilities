...
QBlurRenderPass* QBlurRenderPass::setupDownSample(int val) {
...
void QBlurRenderPass::resizeAndLink(const QSize& size, const TextureLinker& linker) {
	mSrcTexture = linker.getInputTexture(InSlot::Src);
	for (int i = 0; i < 2; i++) {
	...
	linker.registerOutputTexture(OutSlot::Result, "Result", mBlurRT[0].colorAttachment.get());
