...
{
	connect(&p->palette, &ColorPalette::OnColorsChanged, this, &Swatch::paletteModified);
	...
	connect(&p->palette, &ColorPalette::OnColorChanged, [this](int index) {
		if (index == p->selected)
...
void Swatch::mouseMoveEvent(QMouseEvent* event)
{
		...
		(p->drag_pos - event->pos()).manhattanLength() >= QApplication::startDragDistance())
	{
		...
		QPixmap mColorPreview(24, 24);
		mColorPreview.fill(color);
