...
{
	builder.setupTexture(mRT.colorAttachment, "BlurTextureH", QRhiTexture::RGBA8,builder.mainRenderTarget()->pixelSize(), 1, QRhiTexture::RenderTarget | QRhiTexture::UsedAsTransferSource);
	builder.setupRenderTarget(mRT.renderTarget, "BlurRenderTargetH", { mRT.colorAttachment.get() });

	mPainter->setupWindow(builder.renderer()->maybeWindow());
	mPainter->setupPaintFunctor(mInput._PaintFunctor);
	mPainter->setup(builder,mRT.renderTarget);
