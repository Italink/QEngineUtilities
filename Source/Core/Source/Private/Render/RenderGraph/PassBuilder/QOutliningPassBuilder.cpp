...
void QOutliningPassBuilder::refreshParams()
{
	QMatrix4x4 VP = mRenderer->getCamera()->getProjectionMatrixWithCorr() * mRenderer->getCamera()->getViewMatrix();
	mParams.MinSeparation = mInput._MinSeparation;
	mParams.MaxSeparation = mInput._MaxSeparation;
	mParams.MinDistance = mInput._MinDistance;
	mParams.MaxDistance = mInput._MaxDistance;
	mParams.Radius = mInput._Radius;
	mParams.ColorModifier = mInput._ColorModifier;
