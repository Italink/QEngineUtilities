...
void ImGuiPainter::registerImage(const QString& inName, const QImage& inImage) {
	mRegisterImages[inName] = LocalImage({ inImage, nullptr });
}
...
void ImGuiPainter::setup(QRenderGraphBuilder& builder, QRhiRenderTargetRef rt)
{
	if (!mWindow)
		return;

	builder.setupBuffer(mVertexBuffer, "ImGuiKernel", QRhiBuffer::Dynamic, QRhiBuffer::VertexBuffer, sizeof(ImDrawVert) * IMGUI_VERTEX_BUFFER_SIZE);
	builder.setupBuffer(mIndexBuffer, "ImGuiKernel", QRhiBuffer::Dynamic, QRhiBuffer::IndexBuffer, sizeof(ImDrawIdx) * IMGUI_INDEX_BUFFER_SIZE);
	builder.setupBuffer(mUniformBuffer, "ImGuiKernel", QRhiBuffer::Type::Dynamic, QRhiBuffer::UniformBuffer, sizeof(QMatrix4x4));
	...
	builder.setupTexture(mFontTexture, "ImGuiFontTexture", QRhiTexture::RGBA8, QSize(width, height), 1);

	builder.setupShaderResourceBindings(mBindings, "ImGuiBindingsH", {
...
void ImGuiPainter::resourceUpdate(QRhiResourceUpdateBatch* batch, QRhi* rhi) {
	...
	if (io.MouseDrawCursor || (imgui_cursor == ImGuiMouseCursor_None)) {
		...
		QMetaObject::invokeMethod(mWindow, std::bind (&QWindow::setCursor, mWindow, QCursor(shape)));
	}
	ImGui::NewFrame();
	if (mPaintFunctor)
